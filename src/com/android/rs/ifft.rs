#pragma version(1)
#pragma rs java_package_name(com.android.rs)

#include "common.rsh"

uint32_t problemSize = 0;

void root(const float2 *in, float2 *out, const void *usrData, uint32_t i){
  uint32_t p = *(uint32_t *)usrData; // length 
  uint32_t k = i&(p-1);              // index in input sequence, in 0..P-1
  uint32_t j = ((i-k)<<1) + k;       // output index
  float alpha = M_PI*(float)k/(float)p;
  
  
  float2 u0 = in[0];
  float2 u1 = twiddle(in[problemSize],1,alpha);

  
  // In-place DFT-2
  DFT2(u0,u1);
  
  out += j-i;
  
  
  if (p == problemSize){ //last iteration, multiply final result by 1/n
  	u0 = u0 / ((float) problemSize * 2);
  	u1 = u1 / ((float) problemSize * 2);
  }
  
  // Write output
  out[0] = u0;
  out[p] = u1;
}



void runRestricted(rs_script script, rs_allocation in_alloc, rs_allocation out_alloc, uint32_t numElements) {
	 problemSize = numElements / 2;
     struct rs_script_call restrict_for;
     restrict_for.strategy = RS_FOR_EACH_STRATEGY_DONT_CARE;
     restrict_for.xStart = 0;
     restrict_for.xEnd = problemSize;
     restrict_for.yStart = 0;
     restrict_for.yEnd = 0;
     restrict_for.zStart = 0;
     restrict_for.zEnd = 0;
     restrict_for.arrayStart = 0;
     restrict_for.arrayEnd = 0;
     
     uint32_t aIndex = 1; // I/O A initially placed in index 0 after mod operation
     for(uint32_t p = 1; p < numElements; p=p*2){
     	aIndex = (aIndex + 1) % 2;
     	if (aIndex == 0){
     		rsForEach(script, in_alloc, out_alloc, &p, sizeof(uint32_t), &restrict_for); 
     	} else {
     		rsForEach(script,  out_alloc, in_alloc, &p, sizeof(uint32_t), &restrict_for);
     	}
     }
}

